...
test("v7 migration preserves all existing custody rows and rolls back failed DDL", async () => {
  ...
  try {
    ...
    db.exec(
      "INSERT INTO ops_asset_events SELECT * FROM synthetic_old_events; DROP TABLE synthetic_old_events; CREATE INDEX ops_asset_event_history ON ops_asset_events(tenant_id,asset_id,recorded_at,id); DROP TABLE ops_access_events; DROP TABLE ops_access_grants; DROP INDEX ops_application_key; DROP INDEX ops_access_bundle_key; DROP INDEX ops_open_laboratory_case; DROP INDEX ops_purchase_quote_source; DROP INDEX ops_purchase_order_request; DROP INDEX ops_delivery_document_line; DROP INDEX ops_delivery_order; DROP INDEX ops_license_terms_pool; DROP INDEX ops_license_confirmation_source; DROP TABLE ops_asset_imports; DROP TABLE ops_report_previews; DROP INDEX ops_one_current_sales_offer; DROP INDEX ops_one_open_sales_step; DROP INDEX ops_sales_parent; DELETE FROM schema_versions_operations WHERE version>=7;",
    );
    for (const column of [
      "context_json",
      "context_hash",
