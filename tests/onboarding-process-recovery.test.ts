...
test(
  "real SIGKILL after fully evidenced onboarding activation reconciles one committed start after access expires",
  { timeout: 50000 },
  async () => {
    const directory = mkdtempSync(join(tmpdir(), "jarvis-onboarding-process-")),
      children: ReturnType<typeof jsonChild>[] = [],
      script = fileURLToPath(
        new URL("../scripts/onboarding-recovery-child.ts", import.meta.url),
      );
