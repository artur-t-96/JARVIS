...
const childScript = fileURLToPath(
  new URL("../scripts/recovery-child.ts", import.meta.url),
);
...
test(
  "SIGKILL during approval wait and after committed external effect resumes without duplicate",
  { timeout: 30_000 },
  async () => {
    const directory = mkdtempSync(join(tmpdir(), "jarvis-process-test-"));
