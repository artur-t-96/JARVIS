...
const script = fileURLToPath(
  new URL("../scripts/access-recovery-child.ts", import.meta.url),
);
for (const phase of ["attestAccess", "renewAccess", "revokeAccess"])
  test(
    `real SIGKILL after ${phase} reconciles one durable effect even after validity expiry`,
    { timeout: 50000 },
    async () => {
      const directory = mkdtempSync(join(tmpdir(), "jarvis-access-process-")),
      ...
      try {
        ...
        for (let pass = 0; pass < 2; pass++) {
          ...
          assert.deepEqual(await resume.waitFor("finished"), {
            ...
            verified: true,
            grants: 1,
            events: phase === "attestAccess" ? 1 : 2,
            grantVersion: phase === "attestAccess" ? 1 : 2,
            grantStatus: phase === "revokeAccess" ? "revoked" : "active",
            caseChanges: 1,
            receipts: 1,
            executeCalls: 1,
            performedBy: "manager",
