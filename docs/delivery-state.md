...
| P04    | w toku       | Broker i adapter mają testy lokalne; trwa integracja szkicu, okresów i panelu. Brak odbioru całej paczki.                                                                                     |
...
**P04: kontrolowany kontekst firmy i ciągłość rozmowy.** P03 jest odebrane w zakresie minimalnych kontraktów i bramek, P01 pozostaje dostarczone, a pełny odbiór P02/R09 nadal zależy między innymi od rzeczywistego modelu i reguł procesowych. Bieżąca praca łączy broker odczytów, prywatny szkic potrzeby, jawne okresy współpracy oraz ten sam przepływ panelu i rozmowy. Następnie wymagane są zintegrowane testy, Chrome, CI, PR/merge, kopia i aktualizacja wyłącznie zarządzanej instalacji JARVIS. Dalsze P05/P08a/P09a i pozytywny onboarding P06 pozostają osobnymi odbiorami.
...
## P04 — implementacja w toku

Gałąź `codex/company-context`, worktree `/private/tmp/jarvis-company-context`. Baza dostarczenia po rebase to odebrany squash P03 `625d42634c919b94d864d0c8b46817400bedc6ad`; zachowano typowany edytor wymagań, właścicieli spraw oraz poprawkę kończenia procesów OSS. Główna instalacja nadal ma odebrane P03. Nie jest to deklaracja wdrożenia P04.

Istnieją robocze `context-broker.ts` i `business-model.ts`: zamknięte odczyty, trwałe referencje/kursory, bieżące konto i zakresy, limit czterech odczytów / 24 KiB, osobne projekcje lokalne i cloud. Lokalna seria 11 testów brokera oraz 12 testów adaptera przeszła, podobnie jak sprawdzenie backendowego TypeScript. Próby obejmują sześć równoległych okresów ze stronami 5+1, dwa tenanty, zmianę źródła, przeniesienie zadania, unieważnienie dostępu, restart, limit czasu i rozmiaru odpowiedzi oraz rzeczywiste rozliczanie pól usage z odpowiedzi testowego dostawcy. Te 23 testy nie są częścią powyższych 169 testów odebranego P03.

Trwa integracja wersjonowanego szkicu, jawnego wyboru osoby/okresu, polityki krotności współprac, bezpiecznej zamiany referencji na konkretny plan Core i panelu. Następny krok: zamknąć zintegrowane scenariusze P04b/c/e i regresje uprawnień oraz dat, wykonać Chrome, następnie własne CI i dostarczenie. Adapter Anthropic był sprawdzany wyłącznie przez fake fetch; rzeczywisty dostawca i pełny odbiór P04 nie są potwierdzone. Szczegóły i pozostałe warunki: [projekt P04](p04-design.md).

Integracja P04 ma testy trwałego szkicu i klucza, dwóch połączeń, restartu po zapisie Core przed odpowiedzią, cofnięcia konta, zmiany zakresu z anulowaniem niewykonanego planu oraz zachowania zapisanych skutków. Rozmowa pozwala przejść przez jawne wybory osoby, okresu, terminu gotowości, końca rezerwacji, sprawy i sprzętu; listy osób/okresów/spraw mają prywatne kursory. Model otrzymuje wyłącznie strukturalną potrzebę i projekcje brokera, a zaproponowana rezerwacja musi odpowiadać lokalnemu planowi. Limit ośmiu prób modelu w turze przetrwa restart; brak taryfy pozostawia koszt nieznany. Odbiór rzeczywistego Anthropic pozostaje osobny i niepotwierdzony.
